  ...
  return (
    <div className="mt-5 flex flex-wrap px-20 gap-10 mb-20">
      {categories.map((category, index) => (
        <div
          key={index}
          className="flex flex-col items-center gap-2 border-2 border-gray hover:text-white hover:border-white rounded-sm p-4  w-52 h-40 cursor-pointer hover:bg-red-600 hover:shadow-lg transition-all"
          onClick={() => handleCategoryClick(category.name)}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 320 512"
            className="w-52 h-auto fill-current items-center text-black transition duration-300 hover:fill-white"
          >
            <path d={category.icon} />
          </svg>
          <p className="font-roboto ">{category.name}</p>
        </div>
      ))}
    </div>
  );
